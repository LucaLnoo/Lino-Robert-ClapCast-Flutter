    ...
    try{
      ...
      if(title == null){
        for(int page=1; page<pageRange; page++){
          final fetchPeople = await apiCinema.actorService.getPeopleOverviewPage(page);
          people.addAll(fetchPeople.personList);
        }
      }else{
        final fetchPeople = await apiCinema.actorService.searchPeopleOverview(title);
        people.addAll(fetchPeople.personList);
      }

      var filterPeople = people.where((person) => person.profilePathImage?.isNotEmpty ?? false).toList();
      return filterPeople;
    }catch(error){
